...
We found that a fourth-degree polynomial model was the best fit for our data (see @fig-2.5). We predicted the blood lactate threshold at 2 mmol L^-1^ to be 271.8 watts (see @tbl-lactate2) and the blood lactate threshold at 4 mmol L^-1^ to be 314.2 watts (see @tbl-lactate4).
...
#| echo: false
#| label: tbl-lactate2
#| tbl-cap: "Lactate threshold calculations for 2 mmol L^-1^"

library(gt)
lactate_threshold_2 %>% gt() %>%
  tab_options(
...
#| echo: false
#| label: tbl-lactate4
#| tbl-cap: "Lactate threshold calculations for 4 mmol L^-1^"

library(gt)
lactate_threshold_4 %>% gt() %>%
  tab_options(
    table.font.size = px(9), 
    table.width = px(500))
```
